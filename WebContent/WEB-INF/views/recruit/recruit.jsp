...
<script type="text/javascript">
	$j(document).ready(function(){
		...
		var emailReg = /[a-z0-9]+@[a-z]+\.(edu)\.[a-z]{2,3}/;
		//***********dateFormat을 만드는 함수 **************************
		function formatDate($table,fieldName) {
		    ...
		    if(numberInput.includes("-")){
			    ...
			    if (year < 1900 || year > new Date().getFullYear()) {	
			    	alert("fieldName: " + fieldName + "리턴값: -2");
			        return -2;
			    }
			    if (month < 1 || month > 12) {		
			    	alert("fieldName: " + fieldName + "리턴값: -1" + "위치: month");
			        return -1;
			    }
			    if (day < 1 || day > 31) {	
			    	alert("fieldName: " + fieldName + "리턴값: -1" + "위치: day");
			        return -1;
			    }
			    var maxDay;
			    if (month == 2) {
		...
		function checkEducation(){
			...
			var tableNumbers = [];
			$j('#educationBox table').each(function() {
		        var tableId = $j(this).attr('id');
		        var tableNumber = parseInt(tableId.replace('educationTable', ''));
		        tableNumbers.push(tableNumber);
		    });
			console.log("educationTable의 수 : " + tableNumbers.length);
			console.log("educationTableCnt : " + educationTableCnt);
			for(var i =0; i < tableNumbers.length; i ++){
				
				var $table = $j("#educationTable" + tableNumbers[i]);
				...
				var startPeriod = formatDate($table,"startPeriod");
				var endPeriod = formatDate($table,"endPeriod");
				...
				education.push(
					{	
						...
						seq : seq, eduSeq : eduSeq
					}		
				);
				console.log("i: " +i + " educationTableCnt: " + educationTableCnt);
				if(i +1 ==educationTableCnt){
					console.log("checkEdu 마지막 인덱스 도달 isCheckedEducation: " + isCheckedEducation);
		...
		function checkCareer(){
			...
			var tableNumbers = [];
			$j('#careerBox table').each(function() {
		        var tableId = $j(this).attr('id');
		        var tableNumber = parseInt(tableId.replace('careerTable', ''));
		        tableNumbers.push(tableNumber);
		    });
			...
			for(var i =0; i < tableNumbers.length; i ++){
				...
				var $table = $j("#careerTable" + tableNumbers[i]);
				...
				var carSeq = $table.find('[name="carSeq"]').val();
				...
				career.push(
					{	
						...
						endPeriod : endPeriod, task : task, seq : seq, carSeq : carSeq
					}		
				);
				console.log("checkCareer에서 careerTableCnt: " + careerTableCnt);
				console.log("checkCareer에서 CareerTableLength: " + i);
				
				if(i + 1 == careerTableCnt){
					console.log("if 문 진입 확인: i === careerTableCnt");
					isCheckedCareer = true;
					console.log("isCheckedCareer 값 변화: " + isCheckedCareer);
				}
		...
		function checkCertificate(){
			...
			var tableNumbers = [];
			$j('#certificateBox table').each(function() {
		        var tableId = $j(this).attr('id');
		        var tableNumber = parseInt(tableId.replace('certificateTable', ''));
		        tableNumbers.push(tableNumber);
		    });
			...
			for(var i =0; i < tableNumbers.length; i ++){
				...
				var $table = $j("#certificateTable" + tableNumbers[i]);
				...
				var certSeq = $table.find('[name="certSeq"]').val();
				...
				certificate.push(
					{	
						qualifiName : qualifiName, acquDate : acquDate, organizeName :organizeName
						,seq : seq, certSeq : certSeq
					}		
				);
				if(i + 1 ==  certificateTableCnt){
					isCheckedCertificate = true;					
					console.log("checkCert 마지막 인덱스 도달, isCheckedCertificate: " + isCheckedCertificate);
				}
			}
		}
		//*****************************행 추가 삭제에 대한 로직************************************		
		...
		$j("#careerAdd").on("click",function(){
			...
			if(careerHTML === ""){
				careerTableCnt ++;
				newCareerTable = $j("#careerTable1").clone();
				careerHTML = newCareerTable;					
			} else {
				careerTableCnt ++;
				newCareerTable = careerHTML.clone();			
			}			
			newCareerTable.attr("id", "careerTable" + careerTableCnt);
			newCareerTable.find("[name='selection']").prop("checked",false);
			newCareerTable.find("[name='startPeriod'], [name='endPeriod']").val("");
			newCareerTable.find("[name='compName'], [name='task'],[name='carSeq'] , [name='location']").val("");
			newCareerTable.appendTo("#careerBox").show();
			console.log("careerTableCnt: " + careerTableCnt);
		});
		...
		$j("#certificateAdd").on("click",function(){
			...
			if(certificateHTML === ""){
				certificateTableCnt ++;
				newcertificateTable = $j("#certificateTable1").clone();
				certificateHTML = newcertificateTable;			
			}else{
				certificateTableCnt ++;
				newcertificateTable = certificateHTML.clone();								
			}			
			newcertificateTable.attr("id", "certificateTable" + certificateTableCnt);
			newcertificateTable.find("[name='selection']").prop("checked",false);
			newcertificateTable.find("[name='qualifiName'], [name='acquDate'],[name='certSeq'] , [name='organizeName']").val("");
			newcertificateTable.appendTo("#certificateBox").show();	
			console.log("certificateTableCnt: " + certificateTableCnt);
		});
		
		$j("#educationDel").on("click",function(){	
			var checkedIds = [];
			$j("[id^='educationTable'] [type='checkbox']").each(function(){
				if($j(this).is(":checked")){
					var tableId = $j(this).closest("table").attr("id");
		...
		$j("#careerDel").on("click",function(){	
			var checkedIds = [];
			$j("[id^='careerTable'] [type='checkbox']").each(function(){
				if($j(this).is(":checked")){
					var tableId = $j(this).closest("table").attr("id");
					checkedIds.push(tableId);
				}
		...
		$j("#certificateDel").on("click", function(){
			var checkedIds = []; 
			$j("[id^='certificateTable'] [type='checkbox']").each(function(){
				if($j(this).is(":checked")){
					var tableId = $j(this).closest("table").attr("id");
					checkedIds.push(tableId);
				}
...
//******************************저장 또는 제출*********************************		
		$j("#save").on("click", function(){
			...
			if(isRequired && existCareer()){
				console.log("existCareer 검사 결과가 true인 경우");
				checkCareer();
				if(!isCheckedCareer){
					isRequired = false;
				}			
			...
			var overlapMsg = checkDateOverlap(education,career);
	...
	<table align="center">
		<tr>
			<td align="left">
				<span>
				상태 :
				<c:if test="${savedRecruit.getSubmit() eq null }"></c:if>
		...
		<tr>
			...
			<c:if test="${not empty profile }">
				...
				<tr>
					<td id="academicYears">
						<c:choose>
			                <c:when test="${profile.getEducation().getAcademicYears() >= 48 }">
			                  	  대학교(4년제)
			                </c:when>
			                <c:otherwise>
			                   	 대학교(2년제)
		...
		<tr>
			<td id="educationBox">
			<c:if test="${not empty savedEducation }">
				<c:forEach items="${savedEducation}" var="list" varStatus="loop">
				<table id="educationTable${loop.index +1 }" border="1" >
					<tr>
			...
			<c:if test="${empty savedEducation }">
			
				<table id="educationTable1" border="1" >
					<tr>
						<td><input type="hidden" name="eduSeq"></td>
						<td>재학기간</td>
						<td>구분</td>
						<td>학교명(소재지)</td>
						<td>전공</td>
		...
		<tr>
			...
			<c:if test="${not empty savedCareer}">
				<c:forEach items="${savedCareer}" var="list" varStatus="loop">
					<table id="careerTable${loop.index +1 }" border="1" style="width : 100%">
					<tr>
		...
		<tr>
			...
			<c:if test="${not empty savedCertificate}">
				<c:forEach items="${savedCertificate}" var="list" varStatus="loop">
					<table id="certificateTable${loop.index + 1 }" border="1"  style="width : 100%">
					<tr>
